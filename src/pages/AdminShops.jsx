    ...
    <DashboardLayout title="Shops Directory Management" role="admin">
      ...
      <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm flex flex-col gap-6 animate-fade-in">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 pb-4 border-b border-slate-100">
          <div>
            <h1 className="text-xl font-black text-slate-900">Registered Stores Directory</h1>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
            <div className="relative w-full sm:w-64">
              ...
              <input 
                type="text" 
                placeholder="Search by store name, market, category..."
                value={searchQuery}
            ...
            >
              <option value="all">All Verification Statuses ({shops.length})</option>
              <option value="verified">Verified Only ({shops.filter(s => s.verified).length})</option>
              <option value="pending">Pending Verification ({shops.filter(s => !s.verified).length})</option>
            </select>
          </div>
        </div>
