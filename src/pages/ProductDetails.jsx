...
export default function ProductDetails() {
  ...
  // Dynamic rich selection of related items (same category first, then other items)
  const categoryRelated = products.filter(
    (p) => p.id !== product.id && (p.category === product.category || p.categoryName === product.categoryName)
  );
  const otherRelated = products.filter(
    (p) => p.id !== product.id && !categoryRelated.some((c) => c.id === p.id)
  );
  const relatedProducts = [...categoryRelated, ...otherRelated].slice(0, 8);
  ...
  return (
    <div className="w-full flex flex-col items-center">
      ...
      <div className="md:hidden w-full space-y-4 pb-28 pt-2">
        ...
        {relatedProducts.length > 0 && (
          <section className="space-y-2.5 pt-2">
            <div className="flex justify-between items-baseline">
              ...
              <Link to="/shops" className="text-xs font-bold text-[#056839] hover:underline flex items-center gap-0.5">
                <span>View All</span>
        ...
        <div className="fixed bottom-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-md border-t border-slate-100 p-3 px-4 flex items-center gap-3 shadow-[0_-4px_20px_rgba(0,0,0,0.06)] max-w-md mx-auto">
          <button
            type="button"
          ...
          <button
            ...
            className="py-3 px-6 rounded-2xl bg-[#056839] hover:bg-emerald-800 text-white font-extrabold text-xs sm:text-sm flex items-center justify-center gap-2 transition-all flex-[1.8] shadow-xs cursor-pointer border-none"
          >
        ...
        {relatedProducts.length > 0 && (
          <section className="flex flex-col gap-6 mt-4">
            <div className="flex justify-between items-baseline">
              ...
              <Link to="/shops" className="text-xs font-bold text-[#056839] hover:underline flex items-center gap-1">
                <span>View All</span>
                <ChevronRight className="w-3.5 h-3.5" />
              </Link>
            </div>
