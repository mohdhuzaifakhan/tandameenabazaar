...
export default function Home() {
  ...
  // Filter featured products (slice 8 to fit 2 rows of 4 perfectly)
  const featuredProducts = products.filter(p => p.isFeatured).slice(0, 8);

  // Filter popular shops (rating >= 4.4)
  const popularShops = shops.filter(s => s.rating >= 4.4).slice(0, 4);

  return (
    <div className="w-full py-4 md:py-8 flex flex-col gap-8 md:gap-16">
      
      {/* Hero Section (Premium Light Emerald Gradient) */}
      <section className="relative overflow-hidden rounded-3xl bg-gradient-to-tr from-emerald-50 via-white to-teal-50/30 px-6 py-12 md:px-16 md:py-24 text-slate-800 border border-emerald-100/50 flex flex-col lg:flex-row items-center justify-between gap-10">
        ...
        <div className="relative z-10 max-w-2xl flex flex-col gap-4 md:gap-6 text-center lg:text-left items-center lg:items-start">
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-50 border border-emerald-100 text-[10px] font-bold tracking-wider uppercase w-fit text-emerald-800">
            <i className="fa-solid fa-sparkles text-[10px]"></i> Shop Local. Elevated.
          </span>
          ...
          <p className="text-xs md:text-sm text-slate-500 max-w-md font-medium leading-relaxed">
            Browse premium catalogs from Rampur's most trusted sellers. Inquire instantly and complete checkout via WhatsApp directly with merchants.
          </p>
      ...
      {/* Trust Strip - Responsive grid cols */}
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 py-6 border-y border-slate-100 bg-slate-50/50 rounded-2xl px-6">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <h4 className="font-bold text-slate-800 text-xs">Verified Shops</h4>
            <p className="text-[10px] text-slate-400 font-semibold mt-0.5">100% locally verified</p>
          </div>
        </div>
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <h4 className="font-bold text-slate-800 text-xs">Direct Ordering</h4>
            <p className="text-[10px] text-slate-400 font-semibold mt-0.5">Chat directly with sellers</p>
          </div>
        </div>
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <h4 className="font-bold text-slate-800 text-xs">Zero Commissions</h4>
            <p className="text-[10px] text-slate-400 font-semibold mt-0.5">No added checkout fees</p>
          </div>
        </div>
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <h4 className="font-bold text-slate-800 text-xs">Same-Day Pickup</h4>
            <p className="text-[10px] text-slate-400 font-semibold mt-0.5">Fast local coordination</p>
          </div>
        </div>
      </section>

      {/* Explore Categories - Compact fitting */}
      ...
      <section className="flex flex-col gap-5">
        <div className="flex justify-between items-baseline">
          <div>
            <h2 className="text-lg md:text-xl font-black text-slate-900">Featured Curations</h2>
            <p className="text-[11px] text-slate-500 mt-0.5">Handpicked premium products from verified sellers</p>
          </div>
          <Link to="/shops" className="text-xs font-bold text-emerald-600 hover:text-emerald-700 flex items-center gap-1">
            View All <i className="fa-solid fa-chevron-right text-[9px]"></i>
          </Link>
      ...
      <section className="flex flex-col gap-5">
        <div className="flex justify-between items-baseline">
          <div>
            <h2 className="text-lg md:text-xl font-black text-slate-900">Featured Storefronts</h2>
            <p className="text-[11px] text-slate-500 mt-0.5">Browse physical stores operating in Rampur markets</p>
          </div>
          <Link to="/shops" className="text-xs font-bold text-emerald-600 hover:text-emerald-700 flex items-center gap-1">
            View All <i className="fa-solid fa-chevron-right text-[9px]"></i>
          </Link>
